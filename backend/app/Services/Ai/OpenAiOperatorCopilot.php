    ...
    public function respond(SimulationRun $run, string $question, array $history): array
    {
        ...
        $response = Http::baseUrl(config('services.openai.base_url'))
            ...
            ->post('/responses', [
                ...
                'instructions' => 'You are AMAN Copilot, a concise control-room assistant. Answer the operator using only the supplied live situation. Answer the question directly and immediately: never open with a preamble, and never restate that the data is simulated, estimated, or a rehearsal, because the console already labels every reading that way. Mention uncertainty only where it changes what the operator should do, and put it after the answer, never before it. When action is warranted, you may propose sending one eligible responder to the incident zone, but only using an exact incidentId, zoneId, and responderId combination supplied in eligibleCandidates. Never say an action was dispatched or approved. The operator must review the route and explicitly approve dispatch. If evidence is stale, warn the operator and do not make a dispatch suggestion.',
                'input' => json_encode([
